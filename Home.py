...
st.title("Notas de Avaliação dos Serviços da Prefeitura")
...
querry_bairros = 'SELECT DISTINCT bairro_nome FROM V_Resposta_Nota'
df_bairros = conn.query(querry_bairros, ttl=600)
...
##Criando os filtros
filtro_bairro = st.sidebar.multiselect('Selecione o bairro', sorted(df_bairros['bairro_nome']))
...
if filtro_bairro:
    df_nota=df_nota[df_nota['bairro_nome'].isin(filtro_bairro)]
    df_alocacao=df_alocacao[df_alocacao['nome_bairro'].isin(filtro_bairro)]
...
df_media_ponderada = (df_nota['resposta'] * df_nota['peso']).groupby(df_nota['questao']).sum() / df_nota.groupby('questao')['peso'].sum()
df_media_ponderada = df_media_ponderada.sort_values(ascending= False)
tab_media_ponderada = pd.DataFrame({'questao': df_media_ponderada.index, 'media_ponderada': df_media_ponderada.values}).sort_values('media_ponderada', ascending= True)


tab_media_normal = df_nota.groupby('questao')[['resposta']].mean().sort_values('resposta', ascending= True).reset_index()
tab_media_alocacao = df_alocacao.groupby('questao')[['resposta']].mean().sort_values('resposta', ascending= True).reset_index()
...
base_normal = alt.Chart(tab_media_normal).encode(
    ...
    y=alt.Y('questao', bandPosition=0.4).sort('-x').title(""),
